...
// NOTE: The old client-side admin gate (sessionStorage + VITE_ADMIN_PASSWORD_SHA256)
// has been removed. Admin access now requires a real Lovable Cloud user account
// with the 'admin' role (table public.user_roles, helper has_role()).
// See: src/hooks/useAdminAuth.ts and src/pages/AuthPage.tsx
...
/** @deprecated Admin login moved to Lovable Cloud — see useAdminAuth(). */
export async function adminLogin(_login: string, _password: string): Promise<boolean> { return false; }

/** @deprecated Replaced by useAdminAuth().isAdmin. */
export function isAdminConfigured(): boolean { return true; }

/** @deprecated Replaced by useAdminAuth().isAdmin. */
export function isAdminSessionValid(): boolean { return false; }

/** @deprecated Use useAdminAuth().signOut. */
export function adminLogout(): void { /* no-op */ }

/** @deprecated removed */
export async function verifyAdminAccess(_pin: string): Promise<boolean> { return false; }
